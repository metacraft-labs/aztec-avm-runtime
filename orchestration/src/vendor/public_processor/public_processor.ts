...
import { ForkCheckpoint } from '../../fork_checkpoint.ts';

import { AssertionError } from 'assert';

import type { PublicContractsDBInterface } from '../db_interfaces.ts';
import { PublicTreesDB } from '../public_db_sources.ts';
import type { PublicTxSimulatorInterface } from '../public_tx_simulator_interface.ts';
import { GuardedMerkleTreeOperations } from './guarded_merkle_tree.ts';
...
/**
 * What this class needs of a contracts DB: upstream's own interface, plus the one method above it
 * that `processPrivateOnlyTx` calls.
 *
 * Upstream types the constructor parameter as the CLASS `PublicContractsDB`, whose store is a
 * TypeScript map. This runtime's contract store is RESIDENT inside `avm.wasm` and is reached
 * through `avm_contract_db_*`, so the processor is given an adapter instead — and TypeScript being
 * structural, the narrowing is what makes that a declared substitution rather than an accident.
 * `PublicContractsDB` still satisfies it, so upstream's own class remains usable unchanged.
 */
export type ProcessorContractsDB = PublicContractsDBInterface & { addNewContracts(tx: Tx): void };
...
export class PublicProcessor implements Traceable {
  ...
  constructor(
    ...
    debugLogStore: DebugLogStore = new NullDebugLogStore(),
  ) {
  ...
  private async processTxWithPublicCalls(tx: Tx): Promise<[ProcessedTx, NestedProcessReturnValues[], DebugLog[]]> {
    ...
    const { publicTxEffect, gasUsed, revertCode /*callStackMetadata*/ } = result;
    ...
    // THE PROVING REQUEST IS NOT GENERATED, AND THAT IS M22'S DELIVERABLE RATHER THAN AN OMISSION.
    // Upstream's `PublicProcessor.generateProvingRequest` built an `AvmProvingRequest` here — `{ type: ProvingRequestType.PUBLIC_VM, inputs:
    // new AvmCircuitInputs(hints, publicInputs) }` — out of the two members of `PublicTxResult`
    // this runtime does not consume. This runtime does not prove: there is no honk, no polynomial
    // and no SRS anywhere in the wasm link closure (RI-13, asserted by
    // `verify_wasm_link_closure_excludes_proving`), so a request nothing can serve would be a
    // fabricated field on every processed transaction. `makeProcessedTxFromTxWithPublicCalls`
    // declares that parameter optional, so consumers read it as ABSENT rather than as empty.
    const processedTx = makeProcessedTxFromTxWithPublicCalls(
      tx,
      this.globalVariables,
      undefined /* avmProvingRequest — see above */,
      publicTxEffect,
      gasUsed,
      revertCode,
      revertReason,
