...
// SPIKE SHIM: see gas_compat.ts
import { FALLBACK_TEARDOWN_DA_GAS_LIMIT, FALLBACK_TEARDOWN_L2_GAS_LIMIT } from './gas_compat.js';
...
export class PublicTxSimulationTester extends BaseAvmSimulationTester {
  ...
  public static async create(
    ...
    // SPIKE SWITCH (not in the original tree): upstream's default here is the *differential*
    // Cpp-vs-TS simulator, so the suites labelled "(TS Simulator)" in fact require the native
    // bb-avm-sim / wsdb IPC as well. SPIKE_PURE_TS=1 selects the pure-TypeScript simulator so
    // the TS path can be exercised with no native dependency at all.
