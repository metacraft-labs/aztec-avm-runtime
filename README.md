...
| `orchestration/` | **the seam between Aztec's TypeScript and `avm.wasm`** — `WasmAvmPublicTxSimulator`, upstream's own msgpack encoder called rather than reimplemented, the five named AVM configurations that end the `useCppSimulator` inversion, the copied `ForkCheckpoint` and the telemetry replacement. Aztec's transaction orchestration itself (`public_processor/`, `public_tx_simulator/`, `state_manager/`, the side-effect trace, the DB sources — RI-19..RI-23) is **not vendored here yet**; `@aztec/simulator` is unpublished, source copies live in `drift/` and `spike/`, and M18's outstanding tasks carry it. Its dependencies are four `@aztec/*` packages and nothing else: `@aztec/telemetry-client`, `@aztec/native` and `@aztec/world-state` are deliberately absent, and `verify_no_telemetry_client_in_import_graph` walks the module graph to prove it |
...
| `verification/` | the M0 … M18 verification checks; see `just --list` |
| `carry/` | the **ordered carry set** (`series.json`), the measured carry exposure (`exposure.json`), the last replay onto upstream (`rebase.json`) and the acknowledged overlaps between upstream's own changes and ours (`overlap.json`, pinned to upstream's change by blob id so it expires). `series.json` is the authority for what we carry, in what order, and where each patch stands with upstream |
...
| `WORKER-NODE.md` | the dev node in a Web Worker: what upstream already had (its own worker-hosted wallet's `ApiSchema` protocol, and `comlink`, which its own browser worker uses), the one rejection with the measurement behind it, the 2x2 that makes "a busy main thread does not stall block production" a measurement, the transferable read off the sender's own buffer, and the throttling result M27 could not lend it — M32's write-up |
| `WALLET-BOUNDARY.md` | the wallet protocol boundary: the enumeration that found a tenth near-miss before a line was written, the two answers to "is the provider half separable from `@aztec/pxe`" and why they disagree, upstream's own message types over a third transport, and the null wallet that refuses all sixteen methods by name — M33's write-up |
| `DEV-WALLET.md` | the CodeTracer dev wallet: why a DEBUGGING wallet must be the opposite of a production one, the four-part measurement that says `BaseWallet` cannot be subclassed, the ten methods served and the six refused by name, `transfer_in_public` end to end through a wallet in a browser, and the two places the installed pin disagrees with the anchor — M34's write-up |
