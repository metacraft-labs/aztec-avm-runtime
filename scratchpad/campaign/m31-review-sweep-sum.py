...
"""Summarise a campaign sweep log — and REFUSE to print a total while a hole is open.
    ...
    scratchpad/campaign/m31-sweep-sum.py <sweep.log> [reference.json]
...
REFERENCE = {
    ...
    # DECLARED not-carried with a reason; M31 adds one — the SEVENTH such directory, not the sixth
    # as M31's own log and the standing brief both said — and declares it, and the check makes
    # exactly three assertions per declared entry. 259 -> 262, verified mechanically by M31's
    # review (6 directories at codetracer-specs HEAD, 7 with M31's). M11 is also RED for the ninth
    # upstream move, which is a different fact about the same milestone.
    "m11_note": 0,
    # M31's REVIEW took M31 376 -> 421, in three checks and nothing else:
    #   verify_transpiler_wasm_output_identical_to_native  123 -> 130  (+7)
    #   verify_transpiler_rung1_mapping_survives           114 -> 135  (+21)
    #   verify_transpiler_native_build_unaffected           80 ->  97  (+17)
    # 7 + 21 + 17 = 45.  test_transpiled_contract_registers_and_executes is unmoved at 59.
    "m31": 421,
}
REFERENCE.pop("m11_note")
REFERENCE["m11"] = 262
