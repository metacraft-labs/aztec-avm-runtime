...
"""Summarise a campaign sweep log — and REFUSE to print a total while a hole is open.
    ...
    scratchpad/campaign/m37-sweep-sum.py <sweep.log> [reference.json]
...
def main() -> int:
    ...
    if len(sys.argv) > 2:
        ...
        # ------------------------------------------------------------------
        # A REFERENCE FILE IS AN INSTRUMENT, AND THIS ONE CHECKS ITSELF.
        #
        # `m37rev-reference.json` sat on disk summing to 12,114 while the review log it belongs to
        # declared 12,141 — one milestone apart (m26 313 where the live sweep measures 340) — and
        # nothing noticed, because nothing ever added it up. A reference whose entries disagree with
        # the total it is meant to represent makes every future `delta` wrong, silently and in
        # whichever direction the stale entry points. So the file DECLARES its total and this
        # refuses to run against one that does not add up. It is the cheapest possible instance of
        # this campaign's own rule that a figure nobody re-derives rots.
        declared = ref.pop("_total", None)
        ref.pop("_baseline", None)
        ref.pop("_declared_moves", None)
        ref = {k: v for k, v in ref.items() if not k.startswith("_")}
        if declared is None:
            print("REFERENCE PROBLEM: %s declares no `_total`; refusing to compute a delta "
                  "against a table nobody has added up." % sys.argv[2])
            return 2
        got = sum(ref.values())
        if got != declared:
            print("REFERENCE PROBLEM: %s declares _total %d and its %d entries sum to %d "
                  "(difference %+d). REFUSING." % (sys.argv[2], declared, len(ref), got, got - declared))
            return 2
        print("reference %s: %d milestones summing to its declared _total %d  [self-check OK]"
              % (sys.argv[2], len(ref), declared))
