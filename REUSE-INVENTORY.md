...
- decision: replace
- milestone: M33
- why: The SHAPE is reused and the code is not, and the two halves of that are separate facts. What is reused: the state machine — pending sessions, approve/reject, ECDH on approval, encrypted dispatch through `schemaHasMethod`/`getSchemaParameters`/`parseWithOptionals`, PING/PONG while a request is in flight, terminate — is `iframe_connection_handler.ts`'s, member for member, with `window.addEventListener('message')` replaced by `port.onmessage` and nothing else moved; the message-flow comment in `port_connection_handler.ts` is upstream's, unchanged, because the flow is unchanged. Two divergences are declared and both are STRICTER: the session binds `appId` and a secure message that disagrees is refused by name (upstream leans on the cross-origin check, which a `MessagePort` does not have), and the app manifest is recorded where it arrives so §8.4's disclosure survives a wallet that refuses the call.
