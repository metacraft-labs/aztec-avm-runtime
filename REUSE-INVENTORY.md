...
- rejection-reason: cannot-reach-target: the target is a self-contained browser runtime. `AztecNode` is a network- and archiver-backed interface over a running node; its implementations open sockets and databases, and §8.4 forbids us from exporting anything shaped like a node in the first place. What is built is deliberately not an implementation of that interface but a narrow adapter over our own world state, whose surface is fixed by enumeration rather than allowed to grow: OQ-1's deliverable is to list every `node.*` call reachable from the function body, implement exactly that set, and make any call outside it **throw** (`test_aztec_node_adapter_surface_minimal`). TXE's `state_machine/` is upstream doing the same substitution for the same reason and is read first as prior art.
- confidence: reasoned
- experiment: n/a
...
- decision: build
- milestone: M2
- why: Integration evidence across the whole stack for a handful of programs whose intent is recorded.
