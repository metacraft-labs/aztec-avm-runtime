...
**Twenty instances.** (Five places quote the running total: this line, the two M18 checks
...
M19 added one, and M19's review added one. If you add one, move all five numbers together — M19
wrote "eighteen" into its two new files in the same session it moved the other three to
"nineteen", which is the drift this parenthetical exists to prevent.)
...
Fourteen instances. `honk` ⊂ `chonk`. `world_state` ⊂ `world_state_reference`.
...
Current per-milestone counts. Measured in one sweep, M0-M19, on 2026-08-25, every milestone
exit 0 and **0 failures anywhere** — including M9, which is no longer red (see below):

```
m0 156  m1 141  m2 292  m3 199  m4 218  m5 236  m6 363  m7 287  m8 510  m9 804
m10 450  m11 246  m12 691  m13 458  m14 459  m15 537  m16 223  m17 295  m18 278
m19 176                                                CAMPAIGN TOTAL 7,019
```

M3 (+9), M11 (+36), M15 (+2) each moved for a named commit; check the split before assuming drift.
The M19 review moved four, each for a named reason and each with its per-check split taken:
**M4 216 -> 218** (`test_wasi_sdk_33_catches_inside_wasm` 28 -> 30: the link now pins its PATH);
**M9 798 -> 804** (the flake resolved — 798 was one truncated transcript, 804 is the reference);
**M18 276 -> 278** (`verify_no_telemetry_client_in_import_graph` 26 -> 28: two missing negative
cases); **M19 169 -> 176** (`verify_differential_containment` 34 -> 41: the `@aztec/native` graph
absence could not fail). 2 + 6 + 2 + 7 = 17, and 7,002 + 17 = 7,019.
...
- **Upstream moves.** `upstream/next` has advanced twice mid-campaign and once
  turned an M11 red that was not ours. Distinguish that from a regression.
