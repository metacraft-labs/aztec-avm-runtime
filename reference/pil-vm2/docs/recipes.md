<!-- BEGIN VENDORED-PROVENANCE — generated by `just vendor-headers` (tools/provenance.py); do not hand-edit.
     VENDORED — not our code. Re-vendor rather than editing here.
       ...
       upstream-path:   barretenberg/cpp/pil/vm2/docs/recipes.md
...
_OPEN QUESTION:_ _In BB, field equality is using the recipe with the single relation while zero equality is using the conjunction of two separate relations. Why?_
...
$$ (C*{1,i}, C*{2,i}, C*{3,i}) = (C*{4,j}, C*{5,j}, C*{6,j}) $$
...
For a lot of computations we need more than a row and to relate two contiguous to each other, we have access to the value of a given column on the next row through its shifted polynomial. For a column denoted `c`, its shifted counterpart is denoted by `c'`.
As an example, we can increment (top-down) a column `counter` on the active trace (`sel == 1`) as follows:

```
sel * (counter + 1 - counter') = 0;
```

## Proving System Guarantees Related to Shifted Columns on Trace Boundaries

1. The non-shifted column of a shifted column must be zero on the first row (top), i.e., `C == 0` if `C'` is used in any relation of this trace.
2. A shifted column must be zero on the last row of the trace (`C' == 0`).

Both properties follow from security proof our the proving system [Aztec optimized Gemini version](https://eprint.iacr.org/2025/1793.pdf). On page 13, Property E4 implies 1) and E5 implies 2).

## Contiguous Multi-Rows Computation Trace

We provide a set of relations for a given subtrace which comes in handy when we need to perform
some computations which require multiple (possibly variable) rows. We assume the following boolean selectors:

- `sel` main selector activating a sub-trace.
- `precomputed.first_row` toggling the first row of the trace.
- `start` indicating a row starting the computation.
- `end` indicating the row ending the computation.
...
LATCH_CONDITION is an alias expression and is not a committed column.
